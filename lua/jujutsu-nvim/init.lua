...
local diff_presets = {
  ...
  codediff = function(changes)
    if #changes == 1 then
      ...
      local parent_change
      jj.get_changes_by_ids( { changes[1].change_id .. "-"}, function(changes)
        parent_change = changes
      end)
      vim.cmd(string.format("CodeDiff %s %s", changes[1].commit_sha, parent_change[1].commit_sha))
    else
